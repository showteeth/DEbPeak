...
package: "`r BiocStyle::pkg_ver('DEbPeak')`"
abstract: >
  The goal of `DEbPeak` is to explore, visualize, interpret multi-omics data and unravel the regulation of gene expression by combining RNA-seq with peak-related data (eg: ChIP-seq, ATAC-seq, m6a-seq et al.). It contains nine functional modules: Parse GEO (extract study information, raw count matrix and metadata), Quality Control (QC) on count matrix and samples, Principal Component Analysis (PCA) ,Differential Analysis and Visualization (Volcano Plot, Scatter Plot, MA Plot, Rank Plot, Gene Plot, Heatmap, Pie Plot for peak-related data), Functional Enrichment Analysis (GO enrichment analysis, KEGG enrichment analysis, Gene Set Enrichment Analysis), Integrate RNA-seq with peak-related data (Get consensus peaks, Peak profile plots, Peak annotaion, Integrate RNA-seq with peak-related data, Integrate summary, GO enrichment on integrated results, Find motif on integrated results), Integrate RNA-seq with RNA-seq (Integrate summary, GO enrichment on integrated results), Integrate peak-related data with peak-related data (Integrate summary, GO enrichment on integrated results), Utils (useful functions when dealing with RNA-seq data, including gene name conversion and count normalization). 
output: 
...
`DEbPeak` is an R package distributed on [Github](https://github.com/showteeth/DEbPeak).
...
Once `r Rpackage("DEbPeak")` is installed, it can be loaded by the following command.
...
count.file <- system.file("extdata", "snon_count.txt", package = "DEbPeak")
meta.file <- system.file("extdata", "snon_meta.txt", package = "DEbPeak")
ConductDESeq2(count.matrix.file = count.file, meta.file = meta.file, out.folder="/home/songyabing/R/learn/tmp/DEbPeak/AllInOne",
...
```{bash}
tree /home/songyabing/R/learn/tmp/DEbPeak/AllInOne
```

---------------------

# Predict TFs from RNA-seq
...
```{r debatac_rna_deg_infer_convert, eval=FALSE}
...
```{r debatac_rna_deg_infer_convert_load}
...
# Integrate RNA-seq and ChIP-seq/ATAC-seq
...
```{r debchip_peak_annotation_plot, fig.height = 10, fig.width = 14, fig.align = "center", warning=FALSE}
peak.anno$plots
```
...
```{r debchip_peak_denovo, eval = FALSE}
...
```{r debchip_integrate}
debchip.res = DEbPeak(de.res = debchip.dds.results.ordered, peak.res = peak.anno.df, peak.anno.key = "Promoter", merge.key="SYMBOL", species = "Mouse")
head(debchip.res)
```
...
All above steps are suitable for integrating ATAC-seq and RNA-seq data, but the following step (**Motif enrichment**) is usually unique to integrate ATAC-seq and RNA-seq data.

There are **five categories** for users to choose to perform motif enrichment: `UP`, `Peak`, `DOWN`, `DOWNbPeak`, `UPbPeak`. Here, we will use `UPbPeak` as example.
...
We have integrated RNA-seq and ChIP-seq data in the above steps, now we will continue to integrate ATAC-seq data based on the previous results.

### Prepare ATAC-seq data

#### Consensus peaks

```{r debchip_atac_consensus_peak}
# get consensus peak
...
Check the profle of consensus peaks:
```{r debchip_atac_peak_profile, fig.height = 6, fig.width = 10, fig.align = "center", warning=FALSE}
# peak profile plot
...
Peak annotation with [ChIPseeker](https://bioconductor.org/packages/release/bioc/vignettes/ChIPseeker/inst/doc/ChIPseeker.html):
```{r debchip_atac_peak_annotation_df}
# peak annotation
...
```{r debchip_atac_peak_annotation_plot, fig.height = 10, fig.width = 14, fig.align = "center", warning=FALSE}
atac.peak.anno$plots
```

The above pie plot is not based on `ggplot2`, here we provide function to create pie plot based on `ggplot2`:
```{r debchip_atac_peak_annotation_pie, fig.height = 10, fig.width = 10, fig.align = "center", warning=FALSE}
...
```{r debchip_atac_peak_annotation_denovo, eval = FALSE}
# this step is time consuming!
