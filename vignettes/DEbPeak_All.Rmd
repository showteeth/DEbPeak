...
```{bash}
tree /home/songyabing/R/learn/tmp/dekit/AllInOne
```
...
debchip.fe.results = DEbPeakFE(de.peak = debchip.res, peak.type = "ChIP", gene.type = "ENTREZID", species="Mouse",save = F)
```

#### FE on up-regulated targets 
```{r debchip_integrate_fe_up, warning=FALSE, fig.height = 20, fig.width = 8, fig.align = "center"}
...
```{r debchip_integrate_fe_down, warning=FALSE, fig.height = 20, fig.width = 8, fig.align = "center"}
...
All above steps are suitable for integrating ATAC-seq and RNA-seq data, but the following step (**Find motif**) is usually unique to integrate ATAC-seq and RNA-seq data.
### Find motif
```{r debchip_integrate_motif, eval = FALSE}
# debatac.res: similar to debchip.res
ATAC.motif = FindMotif(inte.res = debatac.res, peak.anno.res = peak.anno.df, gene.key = "SYMBOL",
                       homer.motif.path = '/path/to/findMotifsGenome.pl',
...
#### Functional enrichment
```{r debpeak_integrate_fe}
# functional enrichment on direct targets
debpeak.fe.results = DEbPeakFE(de.peak = debpeak.res, peak.type = "Peak", gene.type = "ENTREZID", species="Mouse",save = F)
```

##### FE on up-regulated targets 
```{r debpeak_integrate_fe_up, warning=FALSE, fig.height = 20, fig.width = 8, fig.align = "center"}
up.debpeak.fe.results=debpeak.fe.results[["UPbPeak"]][["GO"]]
head(up.debpeak.fe.results[["table"]])

up.debpeak.fe.results[["plot"]]
```

##### FE on down-regulated targets
```{r debpeak_integrate_fe_down, warning=FALSE, fig.height = 20, fig.width = 8, fig.align = "center"}
down.debpeak.fe.results=debpeak.fe.results[["DOWNbPeak"]][["GO"]]
head(down.debpeak.fe.results[["table"]])

down.debpeak.fe.results[["plot"]]
```
